...
//	unsigned int &degenerate_faces = *info.degenerate_faces;
	...
	for(z=oz;z<oz+size;z+=sizeMin){
		sidetype bz = (z-oz)/sizeMin;
    #pragma omp parallel num_threads(nthreads) private(y)
    {
    	#pragma omp for schedule(auto) 

    	for(y=oy;y<oy+size;y+=sizeMin){
			sidetype by = (y-oy)/sizeMin;
	...
	for( z=oz;z<oz+size-sizeMin;z+=sizeMin){
		sidetype bz = (z-oz)/sizeMin;
	
		for( y=oy;y<oy+size-sizeMin;y+=sizeMin){
			sidetype by = (y-oy)/sizeMin;
...
//	mesh.faces.push_back(mesh.vertices.size()-3);
	...
	for(sidetype z=oz;z<oz+size;z+=sizeMin){
		sidetype bz = (z-oz)/sizeMin;
		#pragma omp parallel private(xx) // x is changed to xx
    {
		#pragma omp for schedule(auto) 
		
		for(  xx=ox;xx<ox+size;xx+=sizeMin){
			sidetype bx = (xx-ox)/sizeMin;
	...
	//				leaf=BRANCHINIT
					...
					sidetype lxl = (xx-lo.x)/ls;
					sidetype lxr = (xx-lo.x)%ls;
					float rx = (float)lxr/(float)ls;
					sidetype lxh = lxl+(rx>0.0 && lxl<bl-1);
